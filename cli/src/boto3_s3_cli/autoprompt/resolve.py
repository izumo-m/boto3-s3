...
docs/imports.md). The interactive prompt itself (:mod:`.prompt`) stays a lazy,
opt-in import.
"""
...
# also declared on the parser for --help; the dispatcher strips them before
# re-dispatching a completed command line).
...
def _active_profile(raw_argv: list[str]) -> str:
    ...
    The env ordering is :data:`~boto3_s3_cli.globalargs.PROFILE_ENV_VARS` (the
    single home of aws's AWS_PROFILE > AWS_DEFAULT_PROFILE rule). Unlike
