...
matcher needs no source/destination path: a relative pattern matches
``info.compare_key`` and a root-anchored (absolute) pattern is anchored against
``info.key`` (the full path) at match time, which is what lets the one filter
prune ``sync``'s two sides per-side the way aws-cli's per-side joining does.
"""
...
def _as_file_filter(matcher: Matcher) -> FileFilter:
    ...
    ``Storage.scan`` stamps ``info.compare_key`` on each entry; the wrapper
    passes it plus ``info.key`` (the full path, the
    anchor for an absolute pattern).
    ``compare_key`` is always set in a filter context;
...
def compile_filter(patterns: list[GlobPattern] | None) -> FileFilter | None:
    ...
    No source/destination path is needed: ``boto3_s3.globsieve`` matches a
    relative pattern against ``info.compare_key`` and a root-anchored (absolute)
    one against ``info.key`` (joined with the entry's drive / UNC anchor at match
    time the way aws-cli joins each pattern onto that side's path). The same
    filter thus prunes both ``sync`` sides per-side, and ``rm`` / ``cp`` / ``mv``
