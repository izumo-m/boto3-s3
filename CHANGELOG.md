...
- A directory replaced or removed while a recursive walk is running is now warned and skipped like `aws s3` instead of failing the whole scan.
...
- More aws-cli parity on transfer wording and shapes: a single-file copy to the bare `s3:///` root keeps the explicit slash and names its destination key like aws, and a rewritten `HeadObject` 404 keeps botocore's retry note.
