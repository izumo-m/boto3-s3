...
Pins ``ETagFilter``'s construction (the ``s3`` / ``part_size`` / ``check_size``
...
class _FakeS3:
    """Minimal stand-in exposing only what ``ETagFilter`` calls on an ``S3``."""
...
class TestConstruction:
    ...
    def test_explicit_part_size(self) -> None:
        assert ETagFilter(part_size=16 * _MIB).part_size == 16 * _MIB

    def test_s3_derives_part_size_from_profile(self) -> None:
        fake = _FakeS3(32 * _MIB)
        f = ETagFilter(fake)  # pyright: ignore[reportArgumentType]
    ...
    def test_explicit_part_size_overrides_s3_unconsulted(self) -> None:
        ...
        fake = _FakeS3(32 * _MIB)
        f = ETagFilter(fake, part_size=16 * _MIB)  # pyright: ignore[reportArgumentType]
        assert f.part_size == 16 * _MIB
        assert fake.aws_config_calls == 0

    def test_check_size_flag(self) -> None:
        assert ETagFilter(check_size=False).check_size is False


class TestS3Integration:
    """``ETagFilter(s3)`` against a real boto3 session over a temp config file."""
    ...
    def test_part_size_from_config_file(
        ...
        assert ETagFilter(s3).part_size == 16 * _MIB
    ...
    def test_absent_key_falls_back_to_default(
        ...
        monkeypatch.setenv("AWS_CONFIG_FILE", str(cfg))
        s3 = S3(session=boto3.Session())
        assert ETagFilter(s3).part_size == DEFAULT_PART_SIZE
...
class TestCopyDirectEtag:
    ...
    def test_equal_etags_skip(self) -> None:
        ...
        assert ETagFilter()(pair) is False

    def test_differing_etags_copy(self) -> None:
        pair = _pair(OpKind.COPY, src=_s3(etag="abc"), dst=_s3(etag="xyz"))
        assert ETagFilter()(pair) is True

    def test_missing_etag_either_side_copies(self) -> None:
        assert ETagFilter()(_pair(OpKind.COPY, src=_s3(etag=None), dst=_s3(etag="abc"))) is True
        assert ETagFilter()(_pair(OpKind.COPY, src=_s3(etag="abc"), dst=_s3(etag=None))) is True
    ...
    def test_size_collision_guard(self) -> None:
        ...
        assert ETagFilter(check_size=True)(pair) is True
        assert ETagFilter(check_size=False)(pair) is False
...
class TestMultipartReconstruction:
    ...
    def test_real_multipart_match_skips(self, tmp_path: Path) -> None:
        ...
        assert ETagFilter(part_size=5 * _MIB)(pair) is False
    ...
    def test_real_multipart_wrong_hash_copies(self, tmp_path: Path) -> None:
        ...
        assert ETagFilter(part_size=5 * _MIB)(pair) is True
...
class TestSizeCheckToggle:
    ...
    def test_size_check_off_reads_and_raises_on_missing(self, tmp_path: Path) -> None:
        ...
        with pytest.raises(OSError):
            ETagFilter(check_size=False)(pair)
    ...
    def test_unknown_size_does_not_short_circuit(self, tmp_path: Path) -> None:
        ...
        assert ETagFilter(check_size=True)(pair) is False
