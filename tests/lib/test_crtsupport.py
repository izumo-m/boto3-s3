...
class TestDefaultUploadChecksum:
    """aws-cli's CRT lane checksums uploads with CRC64NVME (its bundled
    s3transfer's default) where pip s3transfer stamps CRC32, which
    aws-checksums computes in software - a measured 10% of a 1 GB upload
    against real S3 (benchmarks/RESULTS.md 2026-09-06). The default applies
    only where the installed CRT stack can take it."""

    def test_current_stack_defaults_to_crc64nvme(self) -> None:
        import awscrt.s3

        if "CRC64NVME" not in awscrt.s3.S3ChecksumAlgorithm.__members__ or not hasattr(
            s3transfer_crt, "FULL_OBJECT_CHECKSUM_ARGS"
        ):
            pytest.skip("this CRT stack predates CRC64NVME uploads")
        assert crtsupport.default_upload_checksum_algorithm() == "CRC64NVME"

    def test_none_without_awscrt(self, monkeypatch: pytest.MonkeyPatch) -> None:
        hide_awscrt(monkeypatch)
        assert crtsupport.default_upload_checksum_algorithm() is None

    def test_none_when_awscrt_lacks_the_algorithm(self, monkeypatch: pytest.MonkeyPatch) -> None:
        # awscrt < 0.23.8: S3ChecksumAlgorithm has no CRC64NVME member, and
        # s3transfer would raise a KeyError building the checksum config.
        import enum

        import awscrt.s3

        class OldAlgorithms(enum.IntEnum):
            CRC32C = 1
            CRC32 = 2

        monkeypatch.setattr(awscrt.s3, "S3ChecksumAlgorithm", OldAlgorithms)
        assert crtsupport.default_upload_checksum_algorithm() is None

    def test_none_when_s3transfer_predates_full_object_checksums(
        self, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        # Such an s3transfer hands ChecksumAlgorithm to botocore's serializer
        # rather than the CRT's checksum config (the floor handles none at all).
        monkeypatch.delattr(s3transfer_crt, "FULL_OBJECT_CHECKSUM_ARGS", raising=False)
        assert crtsupport.default_upload_checksum_algorithm() is None
