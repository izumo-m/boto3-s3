...
class TestSyncOpens3Upload:
    ...
    def test_uploads_each_source_entry(self) -> None:
        ...
        client, calls = make_recording_client([_listing(), {}, {}])  # empty dst, 2 PutObject
        S3().sync(
            src, S3Storage("s3://b/dst/", client=client), compare=True, transfer_config=_SERIAL
        )
        assert _ops(calls) == ["ListObjectsV2", "PutObject", "PutObject"]
        assert calls[0].params["Prefix"] == "dst/"
        assert [call.params["Key"] for call in calls[1:]] == ["dst/a.txt", "dst/sub/b.txt"]
    ...
    def test_delete_removes_orphan_s3_keys(self) -> None:
        ...
        client, calls = make_recording_client(
            [_listing(("dst/a.txt", 1), ("dst/orphan.txt", 2)), {}, {}]
        )
        S3().sync(
            src,
            S3Storage("s3://b/dst/", client=client),
            compare=True,
            delete=True,
    ...
    def test_unsorted_source_is_rejected(self) -> None:
        ...
        with pytest.raises(ValidationError) as excinfo:
            S3().sync(src, S3Storage("s3://b/dst/", client=client), transfer_config=_SERIAL)
...
class TestSyncS3openDownload:
    ...
    def test_downloads_each_source_entry(self) -> None:
        ...
        S3().sync(
            S3Storage("s3://b/src/", client=client), dst, compare=True, transfer_config=_SERIAL
        )
    ...
    def test_no_delete_does_not_require_delete_capability(self) -> None:
        ...
        store: dict[str, bytes] = {}
        dst = _NoDeleteMem(store, location="mem://data/")
        client, calls = make_recording_client([_listing(("src/a.txt", 3)), _get_response(b"AAA")])
        S3().sync(
            S3Storage("s3://b/src/", client=client), dst, compare=True, transfer_config=_SERIAL
        )
