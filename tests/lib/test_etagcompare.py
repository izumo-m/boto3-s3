...
def _pair(
    ...
    return SyncPair(key="k", transfer_type=transfer_type, src=src, dst=dst)
...
class TestCopyDirectEtag:
    ...
    def test_equal_etags_skip(self) -> None:
        pair = _pair(TransferType.COPY, src=_s3(etag="abc"), dst=_s3(etag="abc"))
        assert EtagComparison()(pair) is False

    def test_differing_etags_copy(self) -> None:
        pair = _pair(TransferType.COPY, src=_s3(etag="abc"), dst=_s3(etag="xyz"))
        assert EtagComparison()(pair) is True

    def test_missing_etag_either_side_copies(self) -> None:
        assert (
            EtagComparison()(_pair(TransferType.COPY, src=_s3(etag=None), dst=_s3(etag="abc")))
            is True
        )
        assert (
            EtagComparison()(_pair(TransferType.COPY, src=_s3(etag="abc"), dst=_s3(etag=None)))
            is True
        )

    def test_non_s3_side_counts_as_differ(self) -> None:
        # A side that is not an S3FileInfo has no comparable etag -> differ.
        pair = _pair(TransferType.COPY, src=FileInfo(key="k"), dst=_s3(etag="abc"))
        assert EtagComparison()(pair) is True

    def test_reads_no_bytes(self) -> None:
        # Keys point at nothing on disk; the equal-etag skip still holds.
        pair = _pair(
            TransferType.COPY, src=_s3(etag="abc", key="nope"), dst=_s3(etag="abc", key="nope")
        )
        assert EtagComparison()(pair) is False
    ...
    def test_size_collision_guard(self) -> None:
        ...
        pair = _pair(TransferType.COPY, src=_s3(etag="abc", size=10), dst=_s3(etag="abc", size=20))
...
class TestNewAndMissingSides:
    ...
    def test_src_none_raises(self, transfer_type: TransferType) -> None:
        with pytest.raises(ValueError, match="without a source entry"):
            EtagComparison()(_pair(transfer_type, dst=_s3(etag="abc")))


class TestUnsupportedKindsAndSides:
    def test_move_kind_raises(self) -> None:
        pair = _pair(TransferType.MOVE, src=_s3(etag="a", size=10), dst=_s3(etag="b", size=10))
        with pytest.raises(ValueError, match="cannot judge a 'move' pair"):
            EtagComparison()(pair)

    def test_delete_kind_raises(self) -> None:
        pair = _pair(TransferType.DELETE, src=_s3(etag="a", size=10), dst=_s3(etag="b", size=10))
    ...
    def test_upload_non_local_local_side_raises(self) -> None:
        ...
        pair = _pair(TransferType.UPLOAD, src=_s3(etag="a", size=10), dst=_s3(etag="b", size=10))
    ...
    def test_download_non_local_local_side_raises(self) -> None:
        ...
        pair = _pair(TransferType.DOWNLOAD, src=_s3(etag="a", size=10), dst=_s3(etag="b", size=10))
        with pytest.raises(ValueError, match="no local side"):
            EtagComparison()(pair)
...
class TestEmptyFile:
    def test_empty_object_uses_single_part_branch(self, tmp_path: Path) -> None:
        ...
        p = _write(tmp_path, b"")
        pair = _pair(
            TransferType.UPLOAD, src=_local(_key(p), size=0), dst=_s3(etag=_EMPTY_SINGLE, size=0)
        )
        assert EtagComparison()(pair) is False
