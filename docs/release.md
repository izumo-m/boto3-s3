...
`cli/CHANGELOG.md`, and a `boto3-s3-cli-vX.Y.Z` tag; ship both at once by bumping
both and tagging the same commit twice):
...
2. Merge into `main`, keeping the merge commit as a clear release boundary:
   ...
   ```
3. Tag that merge commit and push — **this push is what publishes**:
   ```bash
