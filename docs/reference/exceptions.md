...
unrecognized value for a mode option such as `copy_props` or
`annotation_copy_mode`; a case-fold collision under `CaseConflictMode.ERROR`;
and `stdin` or `stdout` unavailable where a stream needs it. It is also the
class for botocore's `ParamValidationError` — client-side validation, so no
request was sent — and for an S3 4xx other than 403 and 404 whose code is not
