...
| MFA one-time code (`TokenCode` in the STS request dict botocore logs at DEBUG, and the urlencoded query form; single-use and short-lived, but a real-time log collector still captures a live second factor) | `'TokenCode': '123456'` -> `'TokenCode': '***'` | `***`. `SerialNumber` (the MFA device ARN) is not a secret and is kept |
